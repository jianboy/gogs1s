	...
	return vscode.window.showInputBox({
		...
		return getExtensionContext()!.globalState.update('gogs-oauth-token', token || '8bc0feea4ad91b28a9df5645c00f4235b77b4afa').then(() => {
			...
			validateToken(token).then(tokenStatus => {
				if (!silent) {
					if (!tokenStatus.valid) {vscode.window.showErrorMessage('OAuth Token have updated, but it is invalid.');}
					else if (tokenStatus.remaining <= 0) {vscode.window.showWarningMessage('OAuth Token have updated, but the rate limit is exceeded.');}
					else {
						vscode.window.showInformationMessage('OAuth Token have updated.');
					}
				}
